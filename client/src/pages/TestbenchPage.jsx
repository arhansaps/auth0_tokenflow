    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      {results.length > 0 && (
        <div className="card p-6">
          ...
          <p className="text-xs mb-6" style={{ color: 'var(--on-surface-variant)' }}>Click a result to see assertion details</p>
          ...
          <div className="space-y-2">
            {results.map((r, idx) => {
              ...
              return (
                <motion.div key={r.runId || r.id || idx} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <div
                  ...
                  >
                    <M icon={r.status === 'passed' ? 'check_circle' : r.status === 'error' ? 'error' : 'cancel'}
                      style={{ fontSize: 20, color: r.status === 'passed' ? 'var(--success)' : 'var(--error)' }} />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold truncate">{r.scenarioName || r.scenario_name}</p>
                      <p className="text-[10px] font-mono" style={{ color: 'var(--outline)' }}>{r.runId || r.id}</p>
                    </div>
                    <div className="flex items-center gap-3 text-[10px] font-bold">
                      <span style={{ color: 'var(--success)' }}>{passed}✓</span>
                      <span style={{ color: 'var(--error)' }}>{failed}✗</span>
                      <span style={{ color: 'var(--outline)' }}>{r.durationMs || r.duration_ms}ms</span>
                    </div>
                  ...
                  <AnimatePresence>
                    {isExpanded && assertions.length > 0 && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                        ...
                        <div className="p-4 space-y-2 ml-4 border-l-2" style={{ borderColor: 'var(--outline-variant)' }}>
                          {assertions.map((a, ai) => (
